...
void CTFGameRules::Arena_RunTeamLogic(void)
{
	...
	else if (iActivePlayers > 0 || (GetGlobalTFTeam(TF_TEAM_RED) && GetGlobalTFTeam(TF_TEAM_RED)->GetNumPlayers() < 1) || (GetGlobalTFTeam(TF_TEAM_BLUE) && GetGlobalTFTeam(TF_TEAM_BLUE)->GetNumPlayers() < 1))
	{
		State_Transition(GR_STATE_PREGAME);
	}
}
...
*/
	...
	if ((collisionGroup1 == TFCOLLISION_GROUP_PUMPKIN_BOMB) &&
			(collisionGroup0 == TFCOLLISION_GROUP_PUMPKIN_BOMB) ||
		(collisionGroup0 == TFCOLLISION_GROUP_ROCKETS))
		return false;
